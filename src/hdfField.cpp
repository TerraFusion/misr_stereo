...
QString hdfFieldNode::Name() const
{
	return name;
}
...
QString hdfFieldNode::DimensionName( int index ) const
{
...
std::list<QStringList> hdfFieldNode::OptionList() const
{
	...
	for ( int dim = 0 ; dim < Rank() ; ++dim )
	{
				...
				&& ( DimensionName( dim ) != "YDim" ) )
		{
			...
			QString prettyName = DimensionName( dim );

			// pretty name : N{X}Dim --> {X}
			if ( prettyName.startsWith( "N" ) && prettyName.endsWith( "Dim" ) )
			{
				prettyName = prettyName.mid( 1, prettyName.length() - 4 );
			}

			// build list for a single option
			for ( int item = 1; item <= DimensionSize( dim ) ; ++item )
			{
				itemList.push_back( prettyName + QString::number( item ) );
			}
...
QString hdfFieldNode::DataTypeString() const
{
	QString dataTypeString = "<undefined>";
...
bool hdfFieldNode::Read( void * dest, int blockMin, int blockMax, int xMin, int xMax, int yMin, int yMax, const std::vector<int> & dims ) const
{
	...
	int32 status = GDreadfield( Grid()->ValidHandle(), Name().toAscii().data(), start, NULL, edge, dest );
...
bool hdfFieldNode::Load( QString fieldName )
{
	name = fieldName;	
	int32 status = FAIL;
	...
	status = GDfieldinfo( Grid()->Handle(), Name().toAscii().data(), & fieldRank, dimensionSizeList, & fieldType, dimensionNameList );
	if ( status == FAIL )
	{
	...
	dimensionName = QString( dimensionNameList ).split( "," );
	...
	// get fill value
	status = GDgetfillvalue( Grid()->Handle(), Name().toAscii().data(), fillValue );
	if ( status == FAIL )
	{
...
void hdfFieldNode::Print( int indent ) const
{
	...
	printf( "%s+ type = %s ( %d bytes )\n", indentString, DataTypeString().toAscii().data(), DataSize() );
	printf( "%s+ dimension[%d] = {", indentString, Rank() );

	for ( int index = 0 ; index < Rank() ; index++ )
	{
		printf( " %s(%d)", DimensionName( index ).toAscii().data(), DimensionSize( index ) );
	}
