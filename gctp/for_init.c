...
-----------------------------------*/
  ...
  if (outsys == UTM)
    ...
    *iflg = obleqforint(radius,center_long,center_lat,shape_m, shape_n, 
    ...
    {
    /* if the sphere value is not 19 or 31 (MODIS ISIN) then call sphdz
       using sphere 31 for MODIS ISIN data
    ------------------------------------------------------------------ */
    if (outspheroid != 19 && outspheroid != 31 )
        sphdz(31,outparm,&r_major,&r_minor,&radius);
